...
> = ({ title, preview, images, featured, slug, types, className }) => (
  <Link href={`/[projectSlug]`} as={`/${slug}`}>
    <a
      ...
      className={clsx(
        "focus group border border-gray-700 bg-gray-800 rounded-md overflow-hidden flex flex-col",
    ...
    >
      {featured && (
        <div className="pb-2/3 bg-gray-100 relative overflow-hidden">
          {images && images[0] && (
            <img
      ...
      <div className="flex flex-1 flex-col justify-between">
        <div className="p-4 pb-0">
          <div className="font-semibold text-gray-200 group-hover:text-teal-300">
            {title}
          </div>
          <div className="text-s"></div>
          <div className={clsx(!featured && "text-sm", "text-gray-500 group-hover:text-teal-400")}>
            {preview}
          </div>
        </div>
...
export const Work: React.FC<{
  ...
  <div className="container my-8">
    <h1 className="text-4xl font-bold text-teal-300">Work</h1>
    <div className="text-2xl text-gray-500">Enterprise UI Design • FullStack Development</div>
